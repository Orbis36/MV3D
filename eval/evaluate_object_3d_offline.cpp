...
const int32_t N_TESTIMAGES = 3767;
...
const double MIN_OVERLAP[3][3] = {{0.5, 0.5, 0.5}, {0.5, 0.25, 0.25}, {0.5, 0.25, 0.25}};
...
tPrData computeStatistics(CLASSES current_class, const vector<tGroundtruth> &gt,
  ...
  for(int32_t i=0; i<gt.size(); i++){
    ...
    // 循环找到一个合适的box，即具有最大iou，j是对detection作出计算
    // 第一次到这里gt有10个框，det只有一个
    ...
    for(int32_t j=0; j<det.size(); j++){
      ...
      // metric is ground; current_class is Car
      int a  = metric;
      int b = current_class;
