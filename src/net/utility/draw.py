...
def draw_box3d_on_camera(rgb, boxes3d, probs = None, color=(255, 0, 255), thickness=1, text_lables=[], calib=None):

    if calib:
        projections, _ = box3d.box3d_to_3drgb_box_3DData(boxes3d, calib, probs)
    else:
    ...
    for i,text in enumerate(text_lables):
        text_pos = (int(np.min(projections[i,:, 0])), max(int(np.min(projections[i,:, 1])), 15) )
