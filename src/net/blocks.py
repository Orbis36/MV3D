...
##  global varaiables ##
IS_TRAIN_PHASE = tf.compat.v1.placeholder(dtype=tf.bool, name='is_train_phase')
                ...
                try:
                    ...
                    if log is not None:
                        log.write(
                            ...
                            (mac, param_size, name, 'Conv2D', oc, h, w, ic, output_name, oh, ow, oc, input_name, ih, iw,
                             ic))
...
def conv2d(input, num_kernels=1, kernel_size=(1, 1), stride=[1, 1, 1, 1], padding='SAME', has_bias=True, name='conv'):
    ...
    w = tf.compat.v1.get_variable(name=name + '_weight', shape=[H, W, C, K],
                        initializer=tf.truncated_normal_initializer(stddev=0.1))
    conv = tf.nn.conv2d(input, w, strides=stride, padding=padding, name=name)
    if has_bias:
        b = tf.compat.v1.get_variable(name=name + '_bias', shape=[K], initializer=tf.constant_initializer(0.0))
        conv = conv + b
...
def avgpool(input, kernel_size=(1, 1), stride=[1, 1, 1, 1], padding='SAME', has_bias=True, is_global_pool=False,
            name='avg'):
...
def linear(input, num_hiddens=1, has_bias=True, name='linear'):
    ...
    w = tf.compat.v1.get_variable(name=name + '_weight', shape=[C, K], initializer=tf.truncated_normal_initializer(stddev=0.1))
...
def upsample2d(input, factor=2, has_bias=True, trainable=True, name='upsample2d'):
    ...
    output_shape = tf.stack([tf.shape(input)[0], tf.shape(input)[1] * factor, tf.shape(input)[2] * factor,
                             tf.shape(input)[3]])  # [N, H*factor, W*factor, C],
    w = tf.compat.v1.get_variable(name=name + '_weight', shape=[size, size, C, K], initializer=init, trainable=trainable)
    deconv = tf.nn.conv2d_transpose(name=name, value=input, filter=w, output_shape=output_shape,
...
def conv2d_bn_relu(input, num_kernels=1, kernel_size=(1, 1), stride=[1, 1, 1, 1], padding='SAME', name='conv'):
    with tf.variable_scope(name) as scope:
        block = conv2d(input, num_kernels=num_kernels, kernel_size=kernel_size, stride=stride, padding=padding,
                       has_bias=False)
        block = bn(block)
        block = relu(block)
    return block
